...
def patch_vlc_data(buf, state):
    ...
    if state['original_url']:
        ...
        if match:
            buf = re.sub('(PLAY|PAUSE|GET_PARAMETER) [^ ]+ RTSP/1.0', '\\1 %s RTSP/1.0' % state['original_url'], buf)
            print "Replaced %s uri.\n  Orig: %s\n  Repl: %s" % (match.groups()[0], match.groups()[1], state['original_url'])
    return buf
...
def handle_client(host, port, vlc):
    ...
    while True:
        rlist, wlist, elist = select.select([stream, vlc], [], [])
        for source in rlist:
            if source is stream:
                source_name = "backend"
                target = vlc
